...
from .helpers import FileDialogUtil
from .snappers import SnapToSolid
...
class PypPlacementInteractor(BaseInteractor):
    ...
    This interactor has three input modes (see InuptMode class). The workflow is as follows:
    ...
    def __init__(self,
        ...
        self.main_palette_service = BuildingElementPaletteService(self.build_ele_list,
                                                                  ...
                                                                  pyp_path)
        self.main_palette_service.show_palette(self.build_ele.script_name)
    ...
    def input_mode(self, value: PypPlacementInteractor.InputMode):
        ...
        if value == self.InputMode.SELECT:
            ...
            self.visual_script_service = VisualScriptService(self.coord_input,
                                                             ...
                                                             self.control_props_list,
                                                             [])
            self.main_palette_service.refresh_palette(self.build_ele_list,
                                                       self.control_props_list)
    ...
    def on_control_event(self, event_id: int):
        ...
        if event_id == 1000:  # button to select a VS-PythonPart
            path = FileDialogUtil.open_file_dialog(self.build_ele.get_existing_property("SelectedPath").value)

            self.build_ele.get_existing_property("SelectedPath").value = path
    ...
    def _init_placement_coord_input(self):
